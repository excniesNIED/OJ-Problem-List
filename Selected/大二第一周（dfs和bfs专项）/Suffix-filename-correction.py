...
def rename_files_in_directory(directory):
    ...
    for filename in os.listdir(directory):
        ...
        # 检查文件是否包含数字前缀
        elif re.match(r'divide\d+\.in', filename) or re.match(r'divide\d+\.out', filename):
            # 构建新的文件名，删除前缀 "problem"
            new_filename = re.sub(r'divide(\d+)\.', r'\1.', filename)
            # 构建完整的文件路径
            old_file_path = os.path.join(directory, filename)
            new_file_path = os.path.join(directory, new_filename)
            # 重命名文件
...
# 指定根目录
root_directory = 'D:\LERNAS\BOT211\OJ\Selected\大二第一周\大二筛题'
